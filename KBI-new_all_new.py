...
correction = 0
if correction==1:
    pp = PdfPages('KBI-corrected_all_new.pdf')
if correction==0:
    pp = PdfPages('KBI_all_new.pdf')
...
system = ["C", "0.9", "A", "1.1", "B"]

for sys in range(len(system)):
    
    # correction = 0
    # if correction==10:
    #     pp1 = PdfPages('RDF+KBI-corrected_A.pdf')
    # if correction==0:
    #     pp1 = PdfPages('RDF+KBI_A.pdf')
    # #fig, axes = plt.subplots(3, 3, figsize=(16, 12))
    ...
    if system[sys] == "A":
        ...
        points = 10
    if system[sys] == "B":
        # x1 = [0.0,0.090050631,0.180455408,0.260996289,0.373349107,0.518857391,0.726329829,0.791892847,0.910002203,1.0]
        x1 = [0.0, 0.103647724, 0.248293855, 0.391093871, 0.472206501, 0.582965203, 0.69269479, 0.786360776, 0.879746995, 1.0]
        points = 10   
    if system[sys] == "C":
        # x1 = [0.0,0.046935022,0.0966771,0.138573913,0.328494051,0.59388573,0.692361506,0.784420007,0.881681146,1.0]
        x1 = [0.0,0.041957881, 0.083981191, 0.122780343, 0.320946777, 0.635060225, 0.740312959, 0.828679361, 0.908626271,1.0]
        points = 10
    if system[sys] == "0.9":
        # x1 = [0.0,0.046935022,0.0966771,0.138573913,0.328494051,0.59388573,0.692361506,0.784420007,0.881681146,1.0]
        x1 = [0.0,0.098861156,0.195589195,0.302326231,0.409434178,0.513391622,
              0.617752297,0.716501537,0.816744499,0.910070916,1.0]
        points = 11
    if system[sys] == "1.1":
        # x1 = [0.0,0.046935022,0.0966771,0.138573913,0.328494051,0.59388573,0.692361506,0.784420007,0.881681146,1.0]
        x1 = [0.0,0.102756392,0.208532889,0.307639691,0.407752446,0.506499966,0.603337202,0.700082331,0.800128515,0.897509657,1.0]
        points = 11
    ...
    for n in range(points):
        ...
        if system[sys] != "0.9" or system[sys] != "1.1":
            #read in the file line by line
            rdf_file = "/scratch365/mfarshad/gpr/md/system"+str(system[sys])+"/lj_cube1/x1="+str(mf)+"/rdf.data"
        if system[sys] == "0.9" or system[sys] == "1.1":
            #read in the file line by line
            rdf_file = "/scratch365/mfarshad/gpr/md/system_"+str(system[sys])+"/lj_cube2/x1="+str(mf)+"/rdf.data"
        ...
        # if mf == str(x1[0]) and mf == str(x1[-1]):
            ...
            # axes[0,0].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[0,1].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[0,2].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[2,0].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[2,1].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[2,2].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[4,0].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[4,1].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            # axes[4,2].set_ylim(-max(ave_rdf_1_1)/10,max(ave_rdf_1_1)*1.5)
            ...
            # axes[0,0].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[0,1].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[0,2].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[3,0].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[3,1].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[3,2].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[6,0].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[6,1].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            # axes[6,2].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='40', loc='upper center', handlelength=0.4)
            ...
            # axes[0,0].set_ylabel(r'$\mathbf{\textit{g (r)}}$', labelpad=45, fontsize=40)
            # axes[3,0].set_ylabel(r'$\mathbf{\textit{g (r)}}$', labelpad=45, fontsize=40)
            # axes[6,0].set_ylabel(r'$\mathbf{\textit{g (r)}}$', labelpad=45, fontsize=40)
        ...
        #      axes[5,0].set_xlim(0,1)
        #      axes[5,1].set_xlim(0,1)
        #      axes[5,2].set_xlim(0,1)
        ...
        #      axes[8,0].set_xlim(0,1)
        #      axes[8,1].set_xlim(0,1)
        ...
        # axes[1,0].set_ylabel(r'$\mathbf{\textit{G$^R (r)$}}$',fontsize=40)
        # axes[4,0].set_ylabel(r'$\mathbf{\textit{G$^R (r)$}}$',fontsize=40)
        # axes[7,0].set_ylabel(r'$\mathbf{\textit{G$^R (r)$}}$',fontsize=40)
        # axes[7,0].set_xlabel(r'$\mathbf{\textit{r}\ (\sigma)}$',fontsize=40)
        # axes[7,1].set_xlabel(r'$\mathbf{\textit{r}\ (\sigma)}$',fontsize=40)
        # axes[7,2].set_xlabel(r'$\mathbf{\textit{r}\ (\sigma)}$',fontsize=40)
    ...
    if system[sys] == "C":
        axes_all[0].errorbar(x2[1:len(x1)], KBI_1_1_all_corrected, yerr=KBI_1_1_all_corrected_error, fmt='o-', color='maroon', markersize=8, fillstyle='none', label=r'$\xi = 0.85$')
        axes_all[1].errorbar(x2[1:len(x1)-1], KBI_1_2_all_corrected, yerr=KBI_1_2_all_corrected_error, fmt='o-', color='maroon', markersize=8, fillstyle='none', label=r'$\xi = 0.85$')
        axes_all[2].errorbar(x2[0:len(x1)-1], KBI_2_2_all_corrected, yerr=KBI_2_2_all_corrected_error, fmt='o-', color='maroon', markersize=8, fillstyle='none', label=r'$\xi = 0.85$')
 
    if system[sys] == "0.9":
        axes_all[0].errorbar(x2[1:len(x1)], KBI_1_1_all_corrected, yerr=KBI_1_1_all_corrected_error, fmt='o-', color='g', markersize=8, fillstyle='none', label=r'$\xi = 0.9$')
        axes_all[1].errorbar(x2[1:len(x1)-1], KBI_1_2_all_corrected, yerr=KBI_1_2_all_corrected_error, fmt='o-', color='g', markersize=8, fillstyle='none', label=r'$\xi = 0.9$')
        axes_all[2].errorbar(x2[0:len(x1)-1], KBI_2_2_all_corrected, yerr=KBI_2_2_all_corrected_error, fmt='o-', color='g', markersize=8, fillstyle='none', label=r'$\xi = 0.9$')
                
    if system[sys] == "A":
        axes_all[0].errorbar(x2[1:len(x1)], KBI_1_1_all_corrected, yerr=KBI_1_1_all_corrected_error, fmt='o-', color='m', markersize=8, fillstyle='none', label=r'$\xi = 1$')
        axes_all[1].errorbar(x2[1:len(x1)-1], KBI_1_2_all_corrected, yerr=KBI_1_2_all_corrected_error, fmt='o-', color='m', markersize=8, fillstyle='none', label=r'$\xi = 1$')
        axes_all[2].errorbar(x2[0:len(x1)-1], KBI_2_2_all_corrected, yerr=KBI_2_2_all_corrected_error, fmt='o-', color='m', markersize=8, fillstyle='none', label=r'$\xi = 1$')

    if system[sys] == "1.1":
        axes_all[0].errorbar(x2[1:len(x1)], KBI_1_1_all_corrected, yerr=KBI_1_1_all_corrected_error, fmt='o-', color='r', markersize=8, fillstyle='none', label=r'$\xi = 1.1$')
        axes_all[1].errorbar(x2[1:len(x1)-1], KBI_1_2_all_corrected, yerr=KBI_1_2_all_corrected_error, fmt='o-', color='r', markersize=8, fillstyle='none', label=r'$\xi = 1.1$')
        axes_all[2].errorbar(x2[0:len(x1)-1], KBI_2_2_all_corrected, yerr=KBI_2_2_all_corrected_error, fmt='o-', color='r', markersize=8, fillstyle='none', label=r'$\xi = 1.1$')
                        
    if system[sys] == "B":
        axes_all[0].errorbar(x2[1:len(x1)], KBI_1_1_all_corrected, yerr=KBI_1_1_all_corrected_error, fmt='o-', color='b', markersize=8, fillstyle='none', label=r'$\xi = 1.2$')
        axes_all[1].errorbar(x2[1:len(x1)-1], KBI_1_2_all_corrected, yerr=KBI_1_2_all_corrected_error, fmt='o-', color='b', markersize=8, fillstyle='none', label=r'$\xi = 1.2$')
        axes_all[2].errorbar(x2[0:len(x1)-1], KBI_2_2_all_corrected, yerr=KBI_2_2_all_corrected_error, fmt='o-', color='b', markersize=8, fillstyle='none', label=r'$\xi = 1.2$')
                
    axes_all[0].set_ylim(min(KBI_2_2_all)*1.2,max(KBI_2_2_all)/-0.15)
    axes_all[1].set_ylim(min(KBI_2_2_all)*1.2,max(KBI_2_2_all)/-0.15)
    axes_all[2].set_ylim(min(KBI_2_2_all)*1.2,max(KBI_2_2_all)/-0.15)

    
    axes_all[0].set_xticks(np.arange(0, 1.1, 0.2))
    axes_all[1].set_xticks(np.arange(0, 1.1, 0.2))
    axes_all[2].set_xticks(np.arange(0, 1.1, 0.2))

    # axes[0,0].set_ylabel(r'$\mathbf{\textit{G$_{11}$}}$',fontsize=40)
    # axes[0,1].set_ylabel(r'$\mathbf{\textit{G$_{12}$}}$',fontsize=40)
    # axes[0,2].set_ylabel(r'$\mathbf{\textit{G$_{22}$}}$',fontsize=40)
    ...
    axes_all[0].set_ylabel(r'$\mathbf{\textit{G}}_{\mathbf{11}}$',fontsize=40)
    axes_all[1].set_ylabel(r'$\mathbf{\textit{G}}_{\mathbf{12}}$',fontsize=40)
    axes_all[2].set_ylabel(r'$\mathbf{\textit{G}}_{\mathbf{22}}$',fontsize=40)
    axes_all[0].set_xlabel(r'$\mathbf{\textit{x}}_{\mathbf{2}}$', fontsize=40)
    axes_all[1].set_xlabel(r'$\mathbf{\textit{x}}_{\mathbf{2}}$', fontsize=40)
    axes_all[2].set_xlabel(r'$\mathbf{\textit{x}}_{\mathbf{2}}$', fontsize=40)

axes_all[0].legend(frameon=False, borderpad=0.1, labelspacing=0.2, columnspacing=0.2, borderaxespad=0.4, handletextpad=0.4, fontsize='18', loc='upper left', handlelength=0.4)

plt.tight_layout()
plt.show()
pp.savefig(fig_all, bbox_inches='tight')
pp.close()
