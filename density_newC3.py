...
pp = PdfPages('density_phase_C3.pdf')
...
for n in range(points):
    ...
    density = "/scratch365/mfarshad/gpr/md/systemC/lj_new3/x1="+str(mf)+"/density_1.T-0.77.data"
    ...
    if mf not in ["0.0", "1.0"]:
        print('den2 =',mf)
        density = "/scratch365/mfarshad/gpr/md/systemC/lj_new3/x1="+str(mf)+"/density_2.T-0.77.data"
    all_density = []  # empty lists for each column
    ...
    if mf == str(x1[8]):
        axes[0,1].plot(xvals,ave_dens,'o',color=colors[8], markersize=8, fillstyle='none', label='x$_{2}$ = '+str(rounded_mf_label))
    ...
    # if mf == str(x1[8]):
    #     axes[0,1].plot(xvals,ave_dens,'o',color=colors[4], markersize=8, fillstyle='none')
...
for n in range(points):
    ...
    #read in the file line by line
    pressure = "/scratch365/mfarshad/gpr/md/systemC/lj_new3/x1="+str(mf)+"/pzz.txt"
    ...
    # #     axes[0].fill_between(xvals, ave_dens - standard_deviations, ave_dens + standard_deviations, color=colors[0], alpha=0.5)
    # if mf == str(x1[8]):
    #     axes[1,0].plot(xvals,ave_press,'o-',color=colors[5], markersize=8, fillstyle='none')
    # #     axes[0].plot(x_fit, y_fit, color=colors[5], label='q = '+str(q))
    # #     axes[0].fill_between(xvals, ave_dens - standard_deviations, ave_dens + standard_deviations, color=colors[0], alpha=0.5)
...
plt.suptitle(r'$\mathbf{\xi = 1}$', fontsize=54, fontweight='bold')
plt.tight_layout()
plt.show()
