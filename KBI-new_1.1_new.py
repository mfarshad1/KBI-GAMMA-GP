...
correction = 0
if correction==10:
    pp1 = PdfPages('RDF+KBI-corrected_1.1_new.pdf')
if correction==0:
    pp1 = PdfPages('RDF+KBI_1.1_new.pdf')
...
for n in range(points):
    ...
    rdf_file = "/scratch365/mfarshad/gpr/md/system_1.1/lj_cube2/x1="+str(mf)+"/rdf.data"
...
correction = 0
if correction==1:
    pp = PdfPages('KBI-corrected_1.1.pdf')
if correction==0:
    pp = PdfPages('KBI_1.1.pdf')
...
# axes[0,0].set_ylim(min(KBI_2_2_all)*1.2,-max(KBI_2_2_all)/0.5)
# axes[0,1].set_ylim(min(KBI_2_2_all)*1.2,-max(KBI_2_2_all)/0.5)
# axes[0,2].set_ylim(min(KBI_2_2_all)*1.2,-max(KBI_2_2_all)/0.5)

# axes[1,0].set_ylim(min(KBI_2_2_all)*1.2,-max(KBI_2_2_all)/0.5)
# axes[1,1].set_ylim(min(KBI_2_2_all)*1.2,-max(KBI_2_2_all)/0.5)
# axes[1,2].set_ylim(min(KBI_2_2_all)*1.2,-max(KBI_2_2_all)/0.5)
